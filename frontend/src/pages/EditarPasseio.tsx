...
export const EditarPasseio: React.FC = () => {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        const [passeioBody, vagBody] = await Promise.all([
          fetch(api.baseUrl + `/passeios/${id}`, {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          }).then(r => r.json()),
          fetch(api.baseUrl + "/usuarios/vagoneteiros?limit=200", {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
          }).then(r => r.json()),
        ]);

        const lista = vagBody.data || vagBody;
        ...
        setApiError("Erro ao carregar dados do passeio");
      }
  ...
  async function handleSalvar() {
    ...
    try {
      ...
      setApiError(err instanceof Error ? err.message : "Erro ao atualizar passeio");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loadingPage) {
    return (
      <div className="min-h-screen bg-bg-light-1 flex items-center justify-center">
        <p className="text-sm text-[#7a8394]">Carregando...</p>
      </div>
    );
  }
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 md:px-8 py-10 flex flex-col gap-0">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden mb-6">
          <div className="flex items-center gap-3 px-6 py-5 border-b border-border">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary mt-0.5">Altere os dados do horário disponível</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-bg-light-2">
            ...
            <button
              onClick={() => navigate("/painel-admin")}
              className="inline-flex items-center gap-2 px-5 h-10.5 rounded-lg border border-border text-text-secondary text-sm font-semibold hover:bg-bg-light-1 hover:border-border-light transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              {loading ? (
                <><span className="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Salvando...</>
              ) : (
                "Salvar Alterações"
              )}
            </button>
